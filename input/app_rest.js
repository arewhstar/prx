...
const fs = require('fs');
const filename = "config_test_out.json";
const outputfilename = "config_test_out_2.json";
   ...
   fs.readFile( __dirname + "/config/" + filename, (err, cfg_rawdata) => {
	...
	req.on('data',(req_rawdata) => {
		...
		if(!exist){
			...
			console.log("EZAZAZAZZ"+existingroutes);
...
//POST ROUTE
// curl -X POST -H "Content-Type: application/json" http://127.0.0.1:8888/routes -d  '{"name":"r2","listener":"l1","cluster":"c1","transform":{"uplink":["c1","c2"],"downlink":[]}}'
